...
rozhraní MONO), `ZADANI-KATALOG.md` + `ZADANI-GEOMETRIE-RM.md` (katalog a
geometrie RM — aktivní priorita), **`DOCS-KATALOG.md`** (stav implementace
katalogu A–E: strom dat, API contract, změny mimo `katalog/`), `ZADANI-ZITRA.md`
(stručný start zítřejší session), `mockup-mono.html` / `mockup-katalog.html`
(vizuální předlohy), `README.md`, `DEPLOY.md`.
...
- **Kód nepiš sám.** Napiš podrobné zadání a předej ho **Grok 4.5**
  (`cursor-grok-4.5-high` / `inherit` — větší úkoly) nebo **Composer 2.5 Fast**
  (drobnosti: i18n, CSS, placeholdery). Hlavní agent zadává a kontroluje.
  Sonnet/Haiku/Opus v tomto projektu **nepoužívat**, dokud zadavatel
...
(async () => {
  for (const f of ['js/arms.js','js/block.js','js/catalog.js',
                   'js/catalog-browser.js','js/custom-dialog.js','js/device-manager.js',
                   ...
                   'katalog/manifest.json','katalog/kategorie.json'])
    await fetch('/'+f, {cache:'reload'}).catch(()=>{});
  // Po změně položky / fotky doplň i konkrétní:
  // katalog/polozky/<id>.json, img/pristroje/<id>-card.*, img/pristroje/<id>-top.*
  location.reload();
})()
```
...
neúplný proplach keše, ne kód. Katalogové `polozky/*.json` a `img/pristroje/*`
do seznamu napevno nedávej (rostou) — po editaci konkrétní položky je
propláchni explicitně (viz komentář ve snippetu). Dokumentace API/stromu:
`DOCS-KATALOG.md`.
...
- **`localStorage.clear()` shodí jazyk na `en`** a české kontrolní řetězce pak
  ...
  Klíče: `alba-katalog-paleta-v1` (id v paletě), legacy `alba-katalog-skryte-v1`
  / `alba-katalog-v1` (loader maže), `nerez-blok-config-v3`, `alba-jazyk`.
...
## B0. Aktuální stav katalogu / geometrie — 8. 8. 2026 (večer)

**Priorita je katalog + RM geometrie.** MONO ostrov a zbývající MONO
nedodělky (ČÁST C úkoly 5–12, ČÁST F) **neřešit**, dokud Jaroslav
výslovně nepřepne prioritu.

### Hotovo dnes (katalog)

- Tovární katalog ze souborů: `katalog/manifest.json`, `katalog/kategorie.json`,
  `katalog/polozky/*.json` — loader v `js/catalog.js`.
- Paleta `alba-katalog-paleta-v1`; overlay e-shopu = **jen správa palety**
  (Přidat / Odebrat / V paletě). Na blok až klikem v paletě po zavření
  overlay. **Žádné šedění.**
- Detail produktu, `zones` + `position`, scroll fix (body lock + `.cat-scroll`),
  i18n (5 jazyků), snapshot v projektu (`factoryVersion` + `catalogSnapshot`).
- **10 položek** v `katalog/polozky/` (8 RM + související), `card.webp` fotky.
- **Top view v detailu — HOTOVO 8. 8. 2026 večer.** Reálná `topImage`
  (webp/jpg/png…) se zobrazí; u SVG placeholderů se generuje SVG půdorys
  z `topFeature` + `zones` (`buildDetailTopView` / `buildTopViewSchematic`
  v `js/catalog-browser.js`, CSS `.cat-detail-top*`). Ověřeno Playwrightem
  na `al-pg28-800-g`: viditelný blok „Top view“, SVG 322×320, 4 hořáky + kW.
  Zítra stačí **propláchnout keš** a potvrdit okem.
- Etapy A–E z `ZADANI-KATALOG.md` jsou prakticky hotové. F/G/H/I čekají
  na pokyn (viz ČÁST C — ZÍTRA).

### Hotovo dnes (geometrie RM)

Schválené typy #1–#5 (verify HTML v kořeni):

| # | typ | verify |
|---|---|---|
| 1–2 | `burners2` | `verify-burners2.html` |
| 3–4 | `burners4` | `verify-burners4.html` |
| 5 | `ceramic4` | `verify-ceramic.html` |
| — | `fryer2` | `verify-fryer2.html` |
| — | `grill` | `verify-grill.html` |

`fryer1` absolutní (F10D / `al-fr10-400-e`) je volitelné pokračování.

### Agenti a pasti z dneška

- Implementace: **Grok 4.5**; drobnosti: **Composer Fast**. Sonnet
  **nepoužívat** bez pokynu. **Ne paralelní zápis do stejného souboru.**
- 3× `python -m http.server` na portu 8000 rozbije HTTP — držet jeden
  server (`127.0.0.1:8000` běží / `alba-konfigurator`).
- Keš ES modulů — vždy proplach ze seznamu v A2 před ověřením.
- Agent „fotky“, který 14 min nic neudělá → zrušit.
- Scroll overlay = `html.cat-overlay-open` + body lock + `.cat-scroll`,
  ne `overflow` na gridu.
- RM fetch bez konkrétních URL trvá věčnost → **raději konkrétní odkazy
  od Jaroslava.**

### Klíčové soubory

`DOCS-KATALOG.md` (implementační dokumentace A–E), `katalog/**`,
`js/catalog.js`, `js/catalog-browser.js`, `js/modules.js`, `js/main.js`,
`js/ui.js`, `js/floorplan.js`, `ZADANI-KATALOG.md`, `ZADANI-GEOMETRIE-RM.md`,
`mockup-katalog.html`, `css/style.css` (`.cat-*`), `js/i18n.js`, `index.html`.
...
- **Přístroje ve 3D** — dříve nedodělek; od 8. 8. 2026 se kreslí (bez výřezu
  v desce). Viz B4 / ČÁST F.
- **Nástavby nad blokem** (roštová nástavba, police na salamandr) — odloženo.
- **Přestavba katalogu** — rozhodnutí uzavřená 8. 8. 2026; etapy A–E
  hotové (viz B0). Zbývá obsah (další produkty), geometrie po vlnách a
  etapy F–I na pokyn — `ZADANI-KATALOG.md`, `ZADANI-GEOMETRIE-RM.md`.
...
## ZÍTRA (priorita — 9. 8. 2026)

Jediný závazný startovací seznam pro zítřejší session. Stručný rozcestník:
`ZADANI-ZITRA.md`. Detail: `ZADANI-KATALOG.md`, `ZADANI-GEOMETRIE-RM.md`.

1. **Ověřit topview v detailu po proplachu keše** (A2). Kód je hotový
   (`buildDetailTopView`); stačí potvrdit okem u `burners4` / `burners2`
   (např. PCD-68G / `al-pg22-800-g` nebo `al-pg28-800-g`).
2. **Další produkty do katalogu** — jen z **konkrétních URL od Jaroslava**
   + geometrie po vlnách se schválením (`ZADANI-GEOMETRIE-RM.md`,
   workflow §6 v `ZADANI-KATALOG.md`).
3. **Volitelně** `fryer1` absolutní (F10D / `al-fr10-400-e`).
4. **Etapy F / G / H / I** z `ZADANI-KATALOG.md` — hloubka varování v pásu,
   pryč device-manager editor, report `publicCode`, dialog `factoryVersion`
   — **až řekne Jaroslav**.
5. **MONO nedodělky** níže (úkoly 5–12, ostrov v ČÁSTI F) — **jen pokud
   Jaroslav přepne prioritu**.

**UX katalogu (závazné):** katalog = Přidat/Odebrat z palety; na blok až
klikem v paletě po zavření; žádné šedění.

**Modely:** Grok 4.5 / Composer Fast; Sonnet nepoužívat bez pokynu.

---

## MONO — starší seznam (platí dál, ale teď mimo prioritu)

Seřazeno podle závažnosti. Nedělá se, dokud zadavatel nepřepne z katalogu.
...
## Úvod — priorita od 8. 8. 2026 večer

**Zítřejší session startuje katalogem**, ne ostrovem. Viz ČÁST C — ZÍTRA a
`ZADANI-ZITRA.md`. MONO plán níže zůstává platný jako backlog.

Plán MONO z 8. 8. 2026 (Krok 0, vlna 1, vlna 2) je **celý hotový**. Ověřené:
přístroje ve 3D, zrcadlení osy X, monolitický vodopád, zkosený vodopád
s půdorysným obrysem, boční kryt 20 mm u zkoseného konce, šipky
přeuspořádání na dlaždice, povrchové úpravy (vyřazení H3 i jejich
diferenciace u podestaveb), drobnosti v pásu, oprava `ZADANI-MONO-UI.md`.
Spodní pás MONO je dodělaný. **Velký zbývající MONO kus = ostrov (úkol 6) —
až na pokyn Jaroslava.**

## Zbývá — úkol 6, ostrovní varianta (až po přepnutí priority)
...
- **Přestavba katalogu** — etapy A–E hotové (B0); obsah + F–I pokračují
  dle `ZADANI-KATALOG.md` / ČÁST C — ZÍTRA. Snapshot v projektu drží
  použité definice.
